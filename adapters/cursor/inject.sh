#!/usr/bin/env bash
# forgein → Cursor adapter
# Fetches your active forgein context and writes it to .cursorrules in the current project.
# Usage: bash inject.sh [--global]
#   --global  writes to ~/.cursor/rules (applies across all projects)
set -euo pipefail
